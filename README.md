...
lintrsecurity provides security-focused [linters](https://en.wikipedia.org/wiki/Lint_(software)) for use with the [lintr](https://lintr.r-lib.org/) package. These are generated automatically from the [pkgaudit](https://github.com/tylerjssmith/lintrsecurity) rules database. The package is intended to support secure R package development. 

Importantly, because lintr respects `# nolint` exclusions, these linters should only be used on code you trust (to remove insecure patterns before sharing). For an alternative implementation that ignores `# nolint` exclusions, see [pkgaudit](https://github.com/tylerjssmith/lintrsecurity).
...
These linters should only be used on code you trust. lintr was built to enforce code style guides, not to ensure security. It therefore will ignore code marked with `# nolint` comments. This is wholly appropriate for a style guide but provides an obvious evasion technique for malicious actors. To scan code with the same rules while ignoring `# nolint` comments, see see [pkgaudit](https://github.com/tylerjssmith/lintrsecurity).

Static analysis also cannot detect all malicious code -- a determined attacker can obfuscate dangerous patterns beyond what any source-level tool can reliably identify. lintrsecurity (or pkgaudit) is just one layer of defense, not a complete solution. Its value is catching naive attacks, surfacing patterns that warrant manual review.

## Installation

You can install lintrsecurity as follows:
